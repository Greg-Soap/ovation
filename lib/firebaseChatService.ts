...
import { notificationServices } from '@/app/(apps)/app-layout'
...
export const storeChatForUsers = async (
  ...
  await setDoc(
    userARef,
    { ...userAChatData, lastMessageSentAt: serverTimestamp() },
    { merge: true },
  )
  await setDoc(
    userBRef,
    { ...userBChatData, lastMessageSentAt: serverTimestamp() },
    { merge: true },
  )
}
...
export const getActiveChatsForUser = async (
  userId: string,
  pageSize = 100,
): Promise<ChatData[]> => {
  try {
...
export const getMessagesForChat = async (
  ...
  const messagesQuery = query(messagesRef, orderBy('timestamp', 'desc'), limit(pageSize))
