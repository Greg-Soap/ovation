      ...
      <section className='container w-full flex flex-col items-center gap-20'>
        <div className='flex flex-col gap-[120px] items-center'>
          <div className='flex flex-col gap-3 w-fit items-center'>
            <p className='text-[#F8F8FF] font-bold text-3xl md:text-5xl text-center'>
              This is where we tell our stories
            </p>
            <p className='text-[#B3B3B3] text-xl md:text-2xl w-full  md:w-[80%] text-center'>
              A space dedicated to sharing our journeys, experiences, and insights. Join us as we
              explore and connect through the power of storytelling.
            </p>
          </div>

          <Link href={`/news/${blogs[0].id}`} className='flex flex-col gap-6 w-full'>
            ...
            <div className='w-full flex flex-col gap-[10px]'>
              ...
              <p className='text-[#CCCCCC] text-lg lg:text-2xl'>{blogs[0].blogDetails}</p>
            </div>
          </Link>
        </div>

        <div className='flex flex-col w-full px-0 py-20 border-y border-[#29292F] gap-6'>
          <p className='text-[#E4E4EC] text-[19px] sm:text-[28px] font-semibold'>Recent blog</p>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 w-full gap-y-20 gap-x-9'>
            {blogs.slice(1).map((blog, index) => (
              <Link
                ...
                key={index}>
                <img src={`${blog.image}`} alt='Blog post' className='w-auto h-[247px]' />
