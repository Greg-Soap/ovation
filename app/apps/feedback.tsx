...
export default function FeedbackModal() {
  ...
  const renderFormHeader = () => (
    <div className="flex flex-col gap-2 mb-6">
      <p className="text-sm text-primary50">{formPage}/2</p>
      <h2 className="text-xl font-semibold text-white100">
  ...
  const renderFirstPage = () => (
    <>
      <FormField
        ...
        render={({ field }) => (
          <FormItem className="flex flex-col w-full">
            <FormLabel className="text-sm text-white100">
              Email address
            </FormLabel>
            <FormControl>
              <Input
                placeholder="Enter your email address"
                className="h-[37px] bg-secondaryBg w-full rounded-[8px] text-xs text-white70"
                type="email"
                {...field}
              />
            </FormControl>
              ...
              >
                {SATISFACTION_OPTIONS.map((option) => (
                  <RadioGroupItem key={option.value} value={option.value}>
                    ...
                    <span className="text-sm text-white70">{option.label}</span>
                  </RadioGroupItem>
                ))}
              </RadioGroup>
            </FormControl>
      ...
      <FormField
        ...
        render={() => (
          <FormItem>
            <FormLabel className="text-sm mb-4 text-white100">
                    ...
                    <FormItem className="flex items-center space-x-3 space-y-0">
                      <FormControl>
                        <Checkbox
                          ...
                          className="border-white70"
                        />
                      </FormControl>
                      <FormLabel className="font-normal text-sm text-white60 leading-none">
                        {item.label}
                      </FormLabel>
                    </FormItem>
                  )}
          ...
          <FormItem className="flex flex-col w-full">
            ...
            <FormControl>
              <Textarea
                ...
                className="max-h-[88px] w-full bg-secondaryBg text-xs text-white70 rounded-[9px]"
                {...field}
              />
            </FormControl>
            <FormMessage />
  ...
  return (
    <div className="px-6 py-8 gap-[30px] bg-popBg rounded-2xl flex flex-col  max-w-[700px] mx-auto overflow-y-scroll overflow-x-hidden h-[90vh] w-[90vw]">
      {renderFormHeader()}
      <Form {...form}>
        <form
        ...
        >
          ...
          <div className="flex justify-end gap-[10px] w-full">
            <Button
              type="button"
              variant="outline"
              className="text-white100 text-[10px] rounded-full font-semibold px-3 py-2 outline outline-1 outline-white15 h-fit bg-transparent"
