...
export default function ExternalEmptyState({ username, type, name }: EmptyState) {
  return (
      ...
      <div className='flex flex-col items-center max-w-[274px] gap-[23px]'>
        <div className='flex flex-col gap-1 items-center'>
          ...
          <p className='text-xs text-[#B3B3B3] text-center'>
            {username} has no {type} yet, send a reminder to them by clicking the button below
          </p>
        </div>

        <Button className='text-xs font-semibold text-[#111115]'>Send a reminder</Button>
      </div>
    </div>
  )
}
