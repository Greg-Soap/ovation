        ...
        <TabsList className="items-center rounded-none px-7 pt-5 pb-0 border-y gap-4 border-white15 w-full h-fit overflow-x-scroll justify-start">
          {profileTabsData.map(({ value, label }) => (
            <TabsTrigger
              key={value}
              value={value}
              className="text-xs text-white60 px-5 py-[10px] bg-transparent border-b-2 border-transparent font-normal data-[state=active]:bg-transparent data-[state=active]:border-button data-[state=active]:text-white100 transition-all duration-300"
            >
              {label}
            </TabsTrigger>
          ))}
