  ...
  return (
    <>
      <div
        ...
        } w-full min-h-[100vh] h-full lg:col-span-1 flex-col other-link border-r border-sectionBorder overflow-auto`}
      >
        ...
        <div className="flex flex-col gap-4 w-full">
          {isLoading ? (
            <div className="flex justify-center items-center h-64">
              <p className="text-white100">Loading chats...</p>
            </div>
          ) : friends.length === 0 ? (
            <div className="flex justify-center items-center h-64">
              <p className="text-white100">No active chats</p>
            </div>
          ) : (
            friends.map((friend, index) => {
              ...
              return (
                <Button
                  className={`flex rounded-none justify-between px-5 py-4 cursor-pointer w-full hover:bg-secondaryBg ${
                    selectedFriend === friend ? 'bg-secondaryBg' : ''
                  }`}
                  key={index}
                  variant="ghost"
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="flex flex-col gap-1 items-start">
                      ...
                      <p className="text-xs text-white70 truncate max-w-[200px]">
                        {friend.lastMessage}
                      </p>
                    </div>
                  </div>
