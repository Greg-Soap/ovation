  ...
  return (
    <div className='flex flex-col w-full bg-[#111115] h-fit items-center justify-center'>
      ...
      <div className='flex flex-col lg:grid lg:grid-cols-3 w-[95%] gap-5'>
        ...
        {!mostViewedLoading ? <DiscoverRight mostViewed={mostViewed} /> : <MiniLoader/>}
      </div>
    </div>
  )
}
...
function GetStarted() {
  const tasks = [
    ...
    {
      title: 'ADD A LINK',
      description: 'Link to your LinkedIn, Twitter, etc. Make Ovation your web3 landing page.',
    ...
    <div className='w-[95%] h-fit rounded-[14px] flex flex-col p-6 border border-[#353538] gap-[30px] mt-10'>
      ...
      <div className='w-full h-fit flex flex-col gap-4'>
        {tasks.map((task, index) => (
          <div key={index} className='flex flex-col lg:flex-row gap-4 bg-[#18181C] border border-[#FFFFFF14] rounded-[10px] items-start lg:items-center justify-between px-5 py-10'>
            <div className='flex items-start lg:items-center gap-4 flex-col lg:flex-row'>
              <div className='flex items-center justify-center rounded-full min-w-11 min-h-11 bg-[#333726]'>
                <img src={task.icon} alt='task icon' className='w-[22px] h-[22px]' />
              </div>
            ...
            <Button asChild className='transition-all duration-300 mt-2 hover:opacity-80'>
              <Link href={task.link} className='text-[10px] text-[#111115] font-medium'>
                {task.buttonText}
              </Link>
            </Button>
          </div>
...
function DiscoverFeature(user: FeaturedUser) {
  return (
    <div className='border border-[#FFFFFF14] rounded-[14px] p-6 gap-9 flex flex-col'>
      ...
      <div id='featured-user' className='flex-col lg:flex-row flex gap-10 items-center w-full'>
        <img
          src={user.img}
        ...
        <div className='flex flex-col max-w-[372px] gap-4'>
          <div className='flex items-center w-full justify-between pb-6 border-b border-[#FFFFFF0D]'>
            <div className='flex items-center gap-2'>
              ...
              <div className='flex flex-col'>
                ...
                <p className='text-xs leading-5 font-medium text-[#B3B3B3]'>@{user.userName}</p>
              </div>
            </div>
          ...
          <p className='text-sm text-[#B3B3B3] pb-6 border-b border-[#FFFFFF0D]'>{user.desc}</p>
          <div className='flex w-full gap-2'>
            {[
              ...
              <p key={index} className='rounded-full border-[1px] text-[10px] py-1 px-2 text-white bg-[#18181C] border-[#353538]'>
  ...
  if (data.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[400px] text-center">
        ...
        <p className="text-lg font-semibold text-white mb-2">No rankings available</p>
        <p className="text-sm text-[#B3B3B3]">Check back later for updated rankings</p>
      </div>
    );
  }
            ...
            <div
              ...
              <div className="flex w-[90%] text-white py-5 px-4 bg-[#1A1A1A] rounded-[18px] mb-10 items-center justify-between border border-[#FFFFFF4D]">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex flex-col">
                    <p className="2xl:text-xl text-sm font-semibold">{user.displayName}</p>
      ...
      <div className="p-4 items-center w-full rounded-lg flex flex-col gap-10 border-[#353538] border-[1px]">
        <Tabs defaultValue="contributors" className="w-full">
          ...
          <TabsContent value="contributors">
            {contributorsQuery.isLoading ? <MiniLoader/> : renderHoldersList(contributorsQuery.data || [])}
          </TabsContent>
          <TabsContent value="creators">
            {creatorsQuery.isLoading ? <MiniLoader/> : renderHoldersList(creatorsQuery.data || [])}
          </TabsContent>
          <TabsContent value="nftHolders">
            {nftHoldersQuery.isLoading ? <MiniLoader/> : renderHoldersList(nftHoldersQuery.data || [])}
          </TabsContent>
          <TabsContent value="blueChipHolders">
            {blueChipHoldersQuery.isLoading ? <MiniLoader/> : renderHoldersList(blueChipHoldersQuery.data || [])}
          </TabsContent>
          <TabsContent value="founderHolders">
            {founderHoldersQuery.isLoading ? <MiniLoader/> : renderHoldersList(founderHoldersQuery.data || [])}
          </TabsContent>
          <TabsContent value="highestNetWorth">
            {highestNetWorthQuery.isLoading ? <MiniLoader/> : renderHoldersList(highestNetWorthQuery.data || [])}
          </TabsContent>
        </Tabs>
      </div>
    </div>
...
const CTabTrigger = ({value, children}: {value: string, children: React.ReactNode}) => {
  return (
    <TabsTrigger value={value} className='text-[10px] font-normal p-2 px-4 rounded-full border-none bg-[#232227]  text-[#999999] data-[state=active]:bg-[#F8F8FF] data-[state=active]:text-[#232227]'>
      {children}
    </TabsTrigger>
  )
}
    ...
    <div className='h-[800px] overflow-scroll mt-10 p-6 mb-5 border border-[#FFFFFF14] rounded-[10px] flex flex-col gap-10 sticky top-1'>
      ...
      <div
        ...
        <div className='flex flex-col items-center justify-center relative'>
          ...
          <Image
            src={mostViewed[0].profileImage || '/assets/images/timeline/Oval.png'}
            alt='User Display'
            width={50}
            height={50}
